export default new (class Platform {
  ...
  constructor() {
    ...
    this.browser = {
      version: (this.uAgent.match(
        /.+(?:me|ox|on|rv|it|era|ie)[\/: ]([\d.]+)/
      ) || [0, '0'])[1],
      ...
      safari:
        !/chrome/i.test(this.uAgent) &&
        /webkit|safari|khtml/i.test(this.uAgent),
      ...
      mobile: /iphone|ipod|ipad|opera mini|opera mobi|iemobile/i.test(
        this.uAgent
      ),
      ...
      android_version:
        parseFloat(this.uAgent.slice(this.uAgent.indexOf('Android') + 8)) || 0,
    }
  }
  ...
  isMobile() {
    if (this.mobileCached === undefined) {
      this.mobileCached = this.getUserAgent().match(
